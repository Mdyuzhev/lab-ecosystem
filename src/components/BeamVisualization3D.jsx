...
// Цвет напряжений по зоне и интенсивности (1 = макс у заделки)
...
// Цвет свечения по зоне
function getGlowColor(zoneColor) {
  if (zoneColor === 'emerald') return 'rgba(16,185,129,0.4)'
  if (zoneColor === 'amber') return 'rgba(245,158,11,0.4)'
  return 'rgba(239,68,68,0.4)'
}

// Сечение: мини-диаграмма
...
function SimpleSupports() {
  ...
  return (
    <g>
      <polygon
        points={`${BEAM_X},${y0} ${BEAM_X - 12},${y0 + triH} ${BEAM_X + 12},${y0 + triH}`}
        fill="#475569" stroke="#64748b" strokeWidth={1}
      />
      <polygon
        points={`${BEAM_X + BEAM_W},${y0} ${BEAM_X + BEAM_W - 12},${y0 + triH} ${BEAM_X + BEAM_W + 12},${y0 + triH}`}
        fill="#475569" stroke="#64748b" strokeWidth={1}
      />
...
export default function BeamVisualization3D({ results, inputs, isAnimating }) {
  ...
  // Прогиб для визуализации
  const maxVisualDeflection = results
    ? Math.min(results.dynamicDeflection * 2, 60)
    : 0

  // --- Пути балки ---
  // Консольная: прогиб от 0 у заделки до max у конца
  // На опорах: max прогиб в центре, 0 по краям

  // Прямая (без прогиба) — фронтальная грань
  ...
  if (results && maxVisualDeflection > 0) {
    if (isCantilever || isSimplified) {
      // Консоль: нарастающий прогиб к свободному концу
  ...
  // Верхняя грань (прямая)
  const straightTopPath = `
    M ${BEAM_X} ${BEAM_Y - BEAM_H / 2}
    C ${BEAM_X + BEAM_W * 0.4} ${BEAM_Y - BEAM_H / 2}
      ${BEAM_X + BEAM_W * 0.7} ${BEAM_Y - BEAM_H / 2}
      ${BEAM_X + BEAM_W} ${BEAM_Y - BEAM_H / 2}
  ...
  if (results && maxVisualDeflection > 0) {
    ...
    if (isCantilever || isSimplified) {
      deflectedTopPath = `
        ...
        C ${BEAM_X + BEAM_W * 0.7 + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D + d * 0.55}
          ${BEAM_X + BEAM_W * 0.4 + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D + d * 0.15}
          ${BEAM_X + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D}
        Z`
    } else if (isSupported) {
      deflectedTopPath = `
        ...
        C ${BEAM_X + BEAM_W * 0.25} ${BEAM_Y - BEAM_H / 2 + d * 0.6}
          ...
          ${BEAM_X + BEAM_W} ${BEAM_Y - BEAM_H / 2}
        L ${BEAM_X + BEAM_W + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D}
        C ${BEAM_X + BEAM_W * 0.75 + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D + d * 0.6}
          ${BEAM_X + BEAM_W * 0.25 + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D + d * 0.6}
          ${BEAM_X + BEAM_D} ${BEAM_Y - BEAM_H / 2 - BEAM_D}
        Z`
    }
  }
  ...
  const endDeflection = results && maxVisualDeflection > 0
    ? (isCantilever || isSimplified ? maxVisualDeflection : 0)
    : 0
  ...
  // Позиция ударника
  const impactX = isCantilever || isSimplified
    ? BEAM_X + BEAM_W
    : BEAM_X + BEAM_W / 2
  const impactBaseY = BEAM_Y - BEAM_H / 2 - BEAM_D
  const impactDeflection = results && maxVisualDeflection > 0
    ? (isCantilever || isSimplified ? maxVisualDeflection : maxVisualDeflection * 0.6)
    : 0
  ...
  return (
    <div className="glass rounded-2xl p-6 mb-6 overflow-hidden">
      <h3 className="font-semibold mb-4 flex items-center gap-2">
        ...
        {isSimplified && (
          <span className="ml-2 text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded">
            Упрощённая модель
          </span>
        )}
      </h3>
      ...
      <div className="bg-slate-950 rounded-xl overflow-hidden">
        ...
        >
          <defs>
            ...
            {/* Градиент напряжений — фронтальная грань */}
            ...
            {/* Градиент — верхняя грань (светлее) */}
          ...
          {/* Фоновая сетка с перспективой */}
          {Array.from({ length: 10 }).map((_, i) => {
            const y = 350 - i * (5 + i * 2)
            return (
              <line key={`grid-${i}`} x1={0} y1={y} x2={800} y2={y}
          ...
          {/* 3D Балка */}
          <g filter={results ? 'url(#beamGlow)' : undefined}>
            {/* Боковая грань (торец) */}
            <motion.path
          ...
          <motion.g
            animate={{
              y: isAnimating ? [-60, impactDeflection, impactDeflection - 10, impactDeflection] : results ? impactDeflection : -60
            }}
            transition={
              isAnimating
          ...
          >
            <rect
              x={impactX - 20} y={impactBaseY - 50}
              width={40} height={35}
              rx={4} fill="#475569" stroke="#64748b" strokeWidth={1}
            />
            <text
              x={impactX} y={impactBaseY - 29}
            ...
            <line
              x1={impactX} y1={impactBaseY - 15}
              x2={impactX} y2={impactBaseY - 4}
              stroke="#f59e0b" strokeWidth={2}
            />
            <polygon
              points={`${impactX - 5},${impactBaseY - 8} ${impactX + 5},${impactBaseY - 8} ${impactX},${impactBaseY}`}
              fill="#f59e0b"
            />
          </motion.g>
          ...
          <g>
            <line
              x1={BEAM_X} y1={BEAM_Y + 60}
              x2={BEAM_X + BEAM_W} y2={BEAM_Y + 60}
          ...
          {results && maxVisualDeflection > 0 && (
            <g>
              {isCantilever || isSimplified ? (
                <>
                  <line
                    x1={BEAM_X + BEAM_W + 25} y1={BEAM_Y}
                    x2={BEAM_X + BEAM_W + 25} y2={BEAM_Y + maxVisualDeflection}
                    stroke={zoneStroke} strokeWidth={1} strokeDasharray="3 3"
                  />
                  <line
                    x1={BEAM_X + BEAM_W + 20} y1={BEAM_Y}
                    x2={BEAM_X + BEAM_W + 30} y2={BEAM_Y}
                    stroke={zoneStroke} strokeWidth={1}
                  />
                  <line
                    x1={BEAM_X + BEAM_W + 20} y1={BEAM_Y + maxVisualDeflection}
                    x2={BEAM_X + BEAM_W + 30} y2={BEAM_Y + maxVisualDeflection}
                    stroke={zoneStroke} strokeWidth={1}
                  />
                  <text
                    x={BEAM_X + BEAM_W + 38} y={BEAM_Y + maxVisualDeflection / 2 + 4}
                    fill={zoneStroke} fontSize="12" fontWeight="bold"
                  >
                ...
                <>
                  <line
                    x1={BEAM_X + BEAM_W / 2 + 25} y1={BEAM_Y}
                    x2={BEAM_X + BEAM_W / 2 + 25} y2={BEAM_Y + maxVisualDeflection * 0.6}
                    stroke={zoneStroke} strokeWidth={1} strokeDasharray="3 3"
                  />
                  <text
                    x={BEAM_X + BEAM_W / 2 + 38} y={BEAM_Y + maxVisualDeflection * 0.3 + 4}
                    fill={zoneStroke} fontSize="12" fontWeight="bold"
                  >
                    δ = {results.dynamicDeflection.toFixed(1)} мм
                  </text>
          ...
          {results && results.ZoneIcon && (
            <g>
              ...
              <text
                x={760} y={375}
                textAnchor="middle" fill={zoneStroke} fontSize="14"
              >
